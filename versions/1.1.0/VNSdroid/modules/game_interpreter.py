...
class VNInterpreter(BoxLayout):
    ...
    skip_mode = BooleanProperty(False)
        ...
        if cmd == "text": 
            ...
            if not self.engine.evaluate_condition(args):
                while True:
                    ...
                    if next_cmd == "fi" or next_cmd is None:
                        break
            self.run_next_command()
            ...
            if self.skip_mode:
                self._finish_bgload(clean_args)
            else:
            ...
            if jump_parts:
                ...
                if new_path:
                    ...
                    if len(jump_parts) > 1:
                        self.engine.local_goto(jump_parts[1])
            self.run_next_command()
        else: 
            self.run_next_command()
    ...
    def handle_text(self, raw_text):
        ...
        if self.full_text and text not in ["~", "!"]:
            ...
            if self.skip_mode:
                ...
                Clock.schedule_once(lambda dt: self.run_next_command(), 0.05)
            else:
    ...
    def handle_setimg(self, args):
        ...
        if filepath and os.path.exists(filepath):
            ...
            img.opacity = 1 if self.skip_mode else 0 # 
            ...
            if pos_mode == 'left':
                ...
                img.pos_hint = {'center_x': 0.5, 'y': 0.0}
            ...
            if not self.skip_mode:
                Animation(opacity=1.0, duration=0.4).start(img)
    ...
    def open_in_game_settings(self):
        ...
        def toggle_disp(instance):
            instance.text = "Full" if instance.text == "Fixed" else "Fixed"
        ...
        def apply_settings(instance):
            try: 
                self.auto_speed = float(speed_input.text)
            except ValueError: 
                pass
